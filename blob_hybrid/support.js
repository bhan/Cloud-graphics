  ...
  function Blob(x, y, radius, numPointMasses)
  {
	...
	this.getBlobParams = function(scaleFactor)
	{

      var i; 
      
      params[0] = this.selected;
      params[1] = this.pointMasses[0].getXPos();
      params[2] = this.pointMasses[0].getYPos(); 
	  params[3] = this.pointMasses.length;

		this.drawBody(null, scaleFactor);
		i = params[3];

        params[i*4+4] = this.middlePointMass.getXPos();
        params[i*4+5] = this.middlePointMass.getYPos();
	    params[i*4+6] = this.radius;
  ...
  function BlobCollective(x, y, startNum, maxNum)
  {
	...
	this.getDrawParams = function(scaleFactor)
	{
		var params = new Array();
		...
		for(i=0; i< this.blobs.length; i++)
		{
			...
			params[i] = this.blobs[i].getBlobParams(scaleFactor);
		}
...
exports.BlobCollective = BlobCollective;
